...
#define KEEP_SECS (365*24*60*60)	//second in one year
...
int create_time_save_file()
{
  ...
  if( fileExists )
  {
    ...
    if( ( fpTime = fopen( "./.piblock", "r+" ) ) == NULL )
    {
  ...
  else                  // if file does not exist...
  {
    ...
    if( ( fpTime = fopen( "./.piblock", "w+" ) ) == NULL )
    {
