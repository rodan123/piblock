  ...
  if( argc > 1 )
  {
    while( ( optChar = getopt( argc, argv, "p:h" ) ) != EOF )
    {
      switch( optChar )
      {
        ...
        case 'h':
        default:
          fprintf( stderr, "Usage: piblock [-p /dev/<portID>]\n" );
...
//for (optChar =0; optChar<40; optChar++){
//	printf("/n");}
 // printf( "\ec" );
ClearScreen();
printf( "%s", copyright );
...
#endif
  printf("(FAX)\n");
  send_modem_command(fd,"AT+FCLASS=1\r");
...
int wait_for_response(fd)
{
  ...
  while(1)
  {
    ...
    // Get the call date from the caller ID string
    if( (dateptr = strstr( buffer, "DATE = " ) ) == NULL )
    {
      strcpy (call_date,"UNKWN");
    } else {
        strncpy(call_date, &dateptr[7], 2 );
        call_date[2] = '/';
        call_date[3] = dateptr[9];
        call_date[4] = dateptr[10];
      }
    call_date[5] = 0;
      
    // Get the call time from the caller ID string
    if( (dateptr = strstr( buffer, "TIME = " ) ) == NULL )
    {
        strcpy (call_time,"UNKWN");
    } else {
        strncpy(call_time, &dateptr[7],2 );
        call_time[2] = ':';
        call_time[3] = dateptr[9];
    ...
    if( check_blacklist( buffer2 ) == TRUE )
    {
      ...
      send_modem_command(fd, "ATH0\r");  // on hook
      usleep( 250000 );               // quarter second
      // Tag and write the call record to the callerID.dat and blacklist.dat
       if( write_blacklist( buffer2, "INTERNET MATCH" ) == TRUE)
          {
            // Tag and write call record to callerID.dat file.
            tag_and_write_callerID_record( buffer2, 'I');
          }
      init_modem(fd);    
      continue;
      }
    else			// start of *-key check
    {
      ...
      if( (nameptr = strstr( buffer2, "NAME = " ) ) == NULL )
      {
        ...
        i = 0;
        while(nameptr[i+7] != '-' && i < 16) {
          buffer[i] = nameptr[i+7];
          i++;
        } 
      }
      buffer[i] = 0;
      ...
      if( TRUE)
      {
        ...
        else if(gotStarKey)
        {
          ...
          if( write_blacklist( buffer2 , "*-KEY ENTRY") == TRUE)
          {
            // Tag and write call record to callerID.dat file.
            tag_and_write_callerID_record( buffer2, '*');
          }
...
static bool check_blacklist( char *callstr )
{
  ...
  while( fgets( blackbuf, sizeof( blackbuf ), fpBl ) != NULL )
  {
    ...
    if (match)
    {
      ...
      // Get the call time from the caller ID string
      if( (dateptr = strstr( callstr, "TIME = " ) ) == NULL )
      {
        strcpy (call_time,"UNKWN");
      } else {
        strncpy(call_time, &dateptr[7],2 );
...
//
bool write_blacklist( char *callstr, char *srcDesc )
{
